 ...
 *
 * Per Sendblue docs: voice notes MUST be .caf to render as native iMessage voice bubbles.
 */
...
async function convertToCaf(mp3Buffer: Buffer): Promise<{ buffer: Buffer; filename: string; mimeType: string }> {
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      execFile('ffmpeg', [
        '-i', inputPath, '-c:a', 'libopus', '-b:a', '24k',
        '-ar', '24000', '-ac', '1', '-f', 'caf', outputPath, '-y',
    ...
    logger.info('[sendblue-voice-memo] Converted to CAF', { inputSize: mp3Buffer.length, outputSize: cafBuffer.length })
    return { buffer: cafBuffer, filename: 'voice-memo.caf', mimeType: 'audio/x-caf' }
  } catch (err) {
    logger.warn('[sendblue-voice-memo] CAF conversion failed, sending as MP3', {
      error: err instanceof Error ? err.message : String(err),
    })
...
export async function sendVoiceMemoBubble(to: string, text: string): Promise<SendResult> {
  ...
  if (!mp3Buffer) return { success: false, error: 'Voice synthesis failed' }
  ...
  if (!mediaUrl) return { success: false, error: 'Media upload failed' }
