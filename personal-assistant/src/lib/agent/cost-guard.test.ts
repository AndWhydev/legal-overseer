...
// ---------------------------------------------------------------------------
// LTV budget scaling tests
// ---------------------------------------------------------------------------

describe('LTV budget scaling', () => {
  describe('getExecutionTokenCap with LTV', () => {
    it('scales per-execution cap by LTV multiplier', () => {
      expect(getExecutionTokenCap('ads', 2.0)).toBe(100_000)
    })

    it('clamps to 10.0 maximum', () => {
      expect(getExecutionTokenCap('ads', 15.0)).toBe(500_000) // 50_000 * 10.0
    })

    it('clamps to 0.1 minimum', () => {
      expect(getExecutionTokenCap('ads', 0.01)).toBe(5_000) // 50_000 * 0.1
    })

    it('returns undefined for unknown roles regardless of LTV', () => {
      expect(getExecutionTokenCap('unknown_role', 5.0)).toBeUndefined()
    })
  })
  ...
  describe('checkRoleBudget with LTV', () => {
    ...
    it('scales daily budget by LTV multiplier', async () => {
      mockGetRoleUsageToday.mockResolvedValue(600_000) // would exceed 500K but not 1M
      const result = await checkRoleBudget(roleMockSupabase, 'org-1', 'ads', 2.0)
      expect(result.allowed).toBe(true)
      expect(result.dailyLimit).toBe(1_000_000) // 500K * 2.0
      expect(result.remainingTokens).toBe(400_000)
    })

    it('returns unbounded for unknown roles regardless of LTV', async () => {
      const result = await checkRoleBudget(roleMockSupabase, 'org-1', 'unknown_role', 5.0)
      expect(result.allowed).toBe(true)
      expect(result.dailyLimit).toBe(Infinity)
    })
  })
  ...
  describe('canProceed with LTV', () => {
    ...
    it('scales daily cost limit by LTV multiplier', async () => {
      mockSupabase.single.mockResolvedValueOnce({
        data: { daily_cost_limit: 10 },
        error: null,
      })
      mockSupabase.gte.mockResolvedValueOnce({
        data: [{ cost_estimate: 25 }], // would exceed $10 but not $30
        error: null,
      })

      const result = await canProceed(mockSupabase as any, 'org-1', 3.0)

      expect(result.allowed).toBe(true)
      expect(result.dailyLimit).toBe(30) // $10 * 3.0
    })

    it('uses base limit when ltvMultiplier is 1.0', async () => {
      mockSupabase.single.mockResolvedValueOnce({
        data: { daily_cost_limit: 50 },
        error: null,
      })
      mockSupabase.gte.mockResolvedValueOnce({
        data: [{ cost_estimate: 20 }],
        error: null,
      })

      const result = await canProceed(mockSupabase as any, 'org-1', 1.0)

      expect(result.dailyLimit).toBe(50) // unchanged
    })
  })
  ...
  describe('backward compatibility', () => {
    ...
    it('existing checkRoleBudget calls without ltvMultiplier work unchanged', async () => {
      mockGetRoleUsageToday.mockResolvedValue(100_000)
      const result = await checkRoleBudget(roleMockSupabase, 'org-1', 'ads')
      expect(result.dailyLimit).toBe(500_000)
    })
