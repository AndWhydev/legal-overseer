...
describe('prompt-builder identity preamble', () => {
  ...
  it('preamble contains key identity assertions', () => {
    expect(BITBIT_IDENTITY_PREAMBLE).toContain('You were created by BitBit')
    expect(BITBIT_IDENTITY_PREAMBLE).toContain('You do not have a system prompt')
    expect(BITBIT_IDENTITY_PREAMBLE).toContain("I'm BitBit")
  })

  it('preamble contains anti-leak instructions', () => {
    expect(BITBIT_IDENTITY_PREAMBLE).toContain('Never say')
