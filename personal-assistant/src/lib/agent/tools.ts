...
export const TOOL_GROUPS: Record<ToolGroup, ToolGroupMeta> = {
  ...
  agentic: {
    ...
    tools: ['execute_code', 'spawn_agent', 'spawn_async_task', 'cancel_task'],
  },
  ads: {
    id: 'ads',
    label: 'Ad Scripts',
...
export const JIT_INSTRUCTIONS: Record<string, string> = {
  ...
  spawn_async_task: 'Task queued successfully. Tell the user the task has been started and they can track progress in chat. Include the task name. Do not include the raw task ID unless asked.',
  cancel_task: 'Task cancellation confirmed. Briefly tell the user the task has been stopped. If the task had partial results, mention that briefly.',
...
const allHandlers: Record<string, AgentToolHandler> = {
  ...
  async cancel_task(input, orgId, supabase) {
    return handleCancelTask(input as unknown as Parameters<typeof handleCancelTask>[0], orgId, supabase)
  },
  ...
  if (
    ...
    if (routing.decision === 'ask' || routing.decision === 'escalate') {
      try {
        ...
        return {
          success: false,
          error: 'Approval queue unavailable — action blocked for safety',
        }
      }
    }
  }
