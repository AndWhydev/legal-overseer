...
import { routeAgentAction, type EntityDelegation } from './confidence-router'
...
export interface QueueAgentActionParams {
  ...
  /** Entity delegation mandate — when present, passed to the confidence router */
  entityDelegation?: EntityDelegation
  /** Entity ID for delegation audit trail logging */
  entity_id?: string
}
...
export async function queueAgentAction(
  ...
  const routing = routeAgentAction(
    ...
    params.entityDelegation,
  )

  // Auto-delegated: bypass approval queue entirely, log to delegation audit trail
  if ((routing.decision as string) === "auto_delegated") {
    const entityId = params.entityDelegation?.entityId ?? params.entity_id
    if (entityId) {
  ...
  if ((routing.decision as string) === "act") {
    return null
  }
  ...
  return createApproval(supabase, {
    ...
    action_summary: params.action_summary,
    confidence_score: params.confidence_score,
    routing_decision: routing.decision as ApprovalRoutingDecision,
