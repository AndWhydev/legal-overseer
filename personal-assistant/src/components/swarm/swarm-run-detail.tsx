...
export function SwarmRunDetail({ runId, onBack, onRollback }: SwarmRunDetailProps) {
  ...
  if (loading) {
    return <div className="flex justify-center p-10 text-muted-foreground text-base">Loading swarm details...</div>;
  }

  if (!run) {
    return <div className="flex justify-center p-10 text-muted-foreground text-base">Swarm run not found</div>;
  }
  ...
  return (
    <div className="flex flex-col gap-5 p-6 h-full overflow-auto">
      {/* Back button */}
      <div
        className="flex items-center gap-2 cursor-pointer text-muted-foreground text-base w-fit hover:text-foreground transition-colors"
        onClick={onBack}
      >
        &larr; Back to swarms
      </div>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="text-base text-muted-foreground mt-1">{run.trigger_input}</div>
          <div className="flex gap-4 flex-wrap mt-1.5">
            <span className="px-3 py-1 rounded-full text-base font-medium bg-muted text-muted-foreground">{run.status.replace('_', ' ')}</span>
            {durationLabel && <span className="px-3 py-1 rounded-full text-base font-medium bg-muted text-muted-foreground">{durationLabel}</span>}
            <span className="px-3 py-1 rounded-full text-base font-medium bg-muted text-muted-foreground">{steps.length} steps</span>
          </div>
          <button
            onClick={() => setShowDetails(d => !d)}
            className="mt-2 px-3 py-1 rounded-lg text-base font-medium bg-muted text-muted-foreground border-none cursor-pointer transition-colors hover:text-foreground"
          >
          ...
          {showDetails && (
            <div className="flex gap-4 flex-wrap mt-2">
              {run.total_cost > 0 && (
                <span className="px-3 py-1 rounded-full text-base font-medium bg-muted text-muted-foreground">${run.total_cost.toFixed(4)}</span>
              )}
        ...
        {canRollback && (
          <button
            className="px-4 py-2 rounded-lg text-base font-medium cursor-pointer border border-red-500 bg-red-500/10 text-red-500 transition-all hover:bg-red-500/20"
            onClick={onRollback}
          >
            Rollback
          </button>
      ...
      {showDetails && (
        <div>
          <div className="text-base font-medium text-muted-foreground uppercase tracking-widest mb-2">Execution Timeline</div>
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <span className={`text-base font-medium ${statusTextColors[step.status] || 'text-muted-foreground'}`}>
                      {STATUS_ICONS[step.status] || STATUS_ICONS.pending}
                    </span>
                    <span className="text-base font-medium text-foreground">{step.step_key.replace(/_/g, ' ')}</span>
                    {step.agent_role && (
                      <span className="text-base font-medium px-2 py-0.5 rounded-lg bg-muted text-muted-foreground">
                        {ROLE_LABELS[step.agent_role] || step.agent_role}
                      </span>
                    )}
                  </div>
                  <span className={`text-base font-medium ${statusTextColors[step.status] || 'text-muted-foreground'}`}>{step.status}</span>
                </div>

                <div className="flex gap-3 mt-1.5 text-base text-muted-foreground">
                ...
                {expandedStep === step.step_key && step.output_data && (
                  <div className="mt-2 p-2 rounded-lg bg-muted text-base tabular-nums text-muted-foreground max-h-[120px] overflow-auto whitespace-pre-wrap break-words">
                ...
                {/* Negotiation display */}
                {step.negotiation && (
                  <div className="mt-2 p-2 rounded-lg bg-muted text-base tabular-nums text-muted-foreground max-h-[120px] overflow-auto whitespace-pre-wrap break-words border-l-2 border-l-purple-500/30">
      ...
      {filteredMessages.length > 0 && (
        <div>
          <div className="text-base font-medium text-muted-foreground uppercase tracking-widest mb-2">Agent Communications ({filteredMessages.length})</div>
          <div className="flex flex-col gap-1.5">
            {filteredMessages.map(msg => (
              <div key={msg.id} className={`p-2 rounded-lg text-base text-muted-foreground ${messageBgColors[msg.message_type] || 'bg-muted'}`}>
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-base font-medium text-muted-foreground uppercase tracking-wider">
                    {msg.from_step_key.replace(/_/g, ' ')}
                  </span>
                  <span className={`text-base font-medium uppercase ${messageTypeColors[msg.message_type] || 'text-muted-foreground'}`}>
                    {msg.message_type}
                  </span>
                  {msg.to_step_key && (
                    <span className="text-base font-medium text-muted-foreground uppercase tracking-wider">
      ...
      {run.result_summary && (
        <div>
          <div className="text-base font-medium text-muted-foreground uppercase tracking-widest mb-2">Result Summary</div>
          <div className="p-3 rounded-xl bg-card text-base text-muted-foreground leading-relaxed whitespace-pre-wrap">
            {run.result_summary}
          </div>
        </div>
      )}
