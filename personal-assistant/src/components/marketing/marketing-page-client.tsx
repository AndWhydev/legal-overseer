    ...
    <div style={{ background: '#0a0a0f', color: '#F1F5F9', overflow: 'hidden' }}>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', background: 'linear-gradient(135deg, #0a0a0f 0%, #0f1419 50%, #141a23 100%)', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, maxWidth: 900, textAlign: 'center' }}>
          <h1 style={{ fontSize: 'clamp(36px, 8vw, 72px)', fontWeight: 700, lineHeight: 1.2, marginBottom: 24, letterSpacing: '-0.03em', color: '#F1F5F9' }}>Your AI operations co-pilot</h1>
          ...
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', marginBottom: 60, flexWrap: 'wrap' }}>
            <Link href="/onboard" className="bb-cta-primary" style={{ padding: '14px 32px', borderRadius: 10, background: '#10b981', color: '#fff', fontSize: 16, fontWeight: 600, textDecoration: 'none', display: 'inline-block', transition: 'all 200ms' }}>Start Free Trial</Link>
            <button className="bb-cta-secondary" style={{ padding: '14px 32px', borderRadius: 10, background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: '#F1F5F9', fontSize: 16, fontWeight: 600, cursor: 'pointer', transition: 'all 200ms' }}>Watch Demo</button>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
            <p style={{ fontSize: 13, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Powered by Claude 3.5 Sonnet</p>
            <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
              {INTEGRATION_LOGOS.slice(0, 6).map((logo) => (
                <div key={logo.name} className="bb-integration-icon" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 44, height: 44, borderRadius: 10, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', fontSize: 20, transition: 'all 200ms' }} title={logo.name}>{logo.icon}</div>
              ))}
            </div>
          </div>
        </div>
      ...
      <section style={{ padding: '100px 20px', position: 'relative', zIndex: 5 }} id="features">
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 80 }}>
            <h2 style={{ fontSize: 'clamp(28px, 5vw, 48px)', fontWeight: 700, marginBottom: 16, letterSpacing: '-0.02em' }}>Why BitBit?</h2>
            <p style={{ fontSize: 18, color: '#94A3B8', maxWidth: 600, margin: '0 auto' }}>Built for digital agencies and operations teams who need AI that understands context and respects boundaries.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 24, marginBottom: 80 }}>
            {FEATURES.map((f) => (
              <div key={f.title} className="bb-feature-card" style={{ padding: '32px 24px', borderRadius: 16, background: 'rgba(15,20,30,0.6)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.03)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05)', transition: 'all 300ms', cursor: 'pointer' }}>
                ...
                <h3 style={{ fontSize: 18, fontWeight: 600, marginBottom: 12, color: '#F1F5F9' }}>{f.title}</h3>
          ...
          <div style={{ padding: '40px 32px', borderRadius: 20, background: 'rgba(10,14,23,0.5)', backdropFilter: 'blur(26px)', WebkitBackdropFilter: 'blur(26px)', border: '1px solid rgba(16,185,129,0.1)', textAlign: 'center' }}>
            <h3 style={{ fontSize: 20, fontWeight: 600, marginBottom: 32, color: '#F1F5F9' }}>20+ Channel Integrations</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(80px, 1fr))', gap: 16 }}>
              {INTEGRATION_LOGOS.map((logo) => (
                <div key={logo.name} className="bb-integration-icon" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 16, borderRadius: 12, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', transition: 'all 200ms' }}>
                  ...
                  <span style={{ fontSize: 12, color: '#94A3B8' }}>{logo.name}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section style={{ padding: '100px 20px', background: 'rgba(5,5,10,0.5)' }} id="pricing">
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 60 }}>
            <h2 style={{ fontSize: 'clamp(28px, 5vw, 48px)', fontWeight: 700, marginBottom: 24, letterSpacing: '-0.02em' }}>Simple, Transparent Pricing</h2>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginTop: 32 }}>
              ...
              {isAnnual && <span style={{ display: 'inline-block', padding: '4px 12px', borderRadius: 20, background: 'rgba(16,185,129,0.15)', color: '#10b981', fontSize: 12, fontWeight: 600, marginLeft: 8 }}>Save 20%</span>}
            </div>
          </div>
            ...
            {PRICING_TIERS.map((tier) => {
              ...
              return (
                <div key={tier.name} className={tier.popular ? 'bb-pricing-popular' : 'bb-pricing-card'} style={{ position: 'relative', padding: '32px 24px', borderRadius: 20, background: tier.popular ? 'linear-gradient(135deg, rgba(16,185,129,0.1) 0%, rgba(16,185,129,0.05) 100%)' : 'rgba(15,20,30,0.6)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: tier.popular ? '1px solid rgba(16,185,129,0.3)' : '1px solid rgba(255,255,255,0.03)', boxShadow: tier.popular ? 'inset 0 1px 0 rgba(16,185,129,0.1), 0 0 40px rgba(16,185,129,0.05)' : 'inset 0 1px 0 rgba(255,255,255,0.05)', transform: tier.popular ? 'scale(1.05)' : 'scale(1)', transition: 'all 300ms' }}>
                  {tier.popular && <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', padding: '6px 16px', borderRadius: 12, background: '#10b981', color: '#000', fontSize: 12, fontWeight: 600 }}>Most Popular</div>}
                  <h3 style={{ fontSize: 20, fontWeight: 600, marginBottom: 8, color: '#F1F5F9' }}>{tier.name}</h3>
                  <p style={{ fontSize: 13, color: '#94A3B8', marginBottom: 24 }}>{tier.description}</p>
                  <div style={{ marginBottom: 32 }}><div style={{ fontSize: 42, fontWeight: 700, color: '#F1F5F9' }}>${displayPrice}<span style={{ fontSize: 16, fontWeight: 400, color: '#94A3B8', marginLeft: 4 }}>{billingPeriod}</span></div></div>
                  <button className={tier.popular ? 'bb-cta-primary' : 'bb-cta-secondary'} style={{ width: '100%', padding: '12px 16px', borderRadius: 10, border: 'none', background: tier.popular ? '#10b981' : 'rgba(255,255,255,0.05)', color: tier.popular ? '#000' : '#F1F5F9', fontSize: 14, fontWeight: 600, cursor: 'pointer', marginBottom: 24, transition: 'all 200ms' }}>Get Started</button>
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 24 }}>
                    {tier.features.map((feature) => (<div key={feature} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', fontSize: 13, color: '#94A3B8' }}><span style={{ color: '#10b981', fontWeight: 'bold', fontSize: 16, lineHeight: 1 }}>✓</span>{feature}</div>))}
                  </div>
                </div>
              )
            })}
      ...
      <section style={{ padding: '80px 20px', textAlign: 'center' }}>
        <h2 style={{ fontSize: 36, fontWeight: 700, marginBottom: 24, color: '#F1F5F9' }}>Ready to transform your operations?</h2>
        ...
        <Link href="/onboard" className="bb-cta-primary" style={{ display: 'inline-block', padding: '14px 40px', borderRadius: 10, background: '#10b981', color: '#fff', fontSize: 16, fontWeight: 600, textDecoration: 'none', transition: 'all 200ms' }}>Start Your Free Trial</Link>
      </section>
