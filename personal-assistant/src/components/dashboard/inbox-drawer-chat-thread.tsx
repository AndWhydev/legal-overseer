            ...
            <div>
              ...
              {hasMedia && msg.attachments!.filter((a: AttachmentItem) => a.type === 'image').map((a: AttachmentItem, i: number) => (
                ...
                >
                  ...
                  <div className="px-2.5 py-1.5 text-sm text-sidebar-foreground/40">
                    {a.name}
                  </div>
                </div>
              ))}
              ...
              {hasMedia && msg.attachments!.filter((a: AttachmentItem) => a.type !== 'image').map((a: AttachmentItem, i: number) => (
                <div key={i} className="mt-1 flex items-center gap-1.5 rounded-lg bg-sidebar-foreground/[0.03] px-2.5 py-1.5 text-sm text-sidebar-foreground/40">
                  📎 {a.name} · {a.size}
                </div>
              ))}
            </div>
