    ...
    <>
        ...
        <div className="shrink-0 px-6 py-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="shrink-0 text-right text-xs text-sidebar-foreground/60">
              {formatDate(message.receivedAt)}
            </div>
          </div>

          {message.subject && (
            <h2 className="mb-2.5 text-base leading-snug font-medium text-sidebar-foreground">
        ...
        {/* Body Content */}
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain flex flex-col gap-4 px-6 py-5 text-sm leading-relaxed text-sidebar-foreground">
          {showSummary && (
            <AiSummaryPanel
              message={message}
              onCreateTask={(data) => {
        ...
        <div className="shrink-0 px-5 py-4">
          <div className="flex items-end gap-2 rounded-xl bg-sidebar-accent/35 px-3 py-2">
            <textarea
              ref={textareaRef}
              value={replyText}
              onChange={(e) => {
              ...
              className="min-h-8 max-h-[200px] w-full flex-1 resize-none border-none bg-transparent text-sm leading-normal font-inherit text-sidebar-foreground outline-none placeholder:text-sidebar-foreground/45"
            />
            <Button
              variant="ghost"
              size="icon-sm"
              ...
              className={replyText.trim() ? 'text-sidebar-foreground' : 'text-sidebar-foreground/40'}
            >
              <IconSend className="size-3.5" />
            </Button>
          </div>
          {replyText && (
            <div className="mt-1.5 pl-3 text-xs text-sidebar-foreground/60">
