  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-sm">Contact health by last interaction</CardDescription>
      </CardHeader>
      <CardContent className="space-y-0">
        {isLoading ? (
          ...
          <div className="text-sm text-muted-foreground py-4">No contacts</div>
        ) : (
          (data?.contacts ?? []).map((c, i) => (
            <div key={i} className="flex gap-3 py-1.5 border-b border-border last:border-0 items-center">
              ...
              <span className="text-sm text-muted-foreground shrink-0 tabular-nums">{c.daysSince === 999 ? 'never' : `${c.daysSince}d`}</span>
            </div>
          ))
        )}
      </CardContent>
