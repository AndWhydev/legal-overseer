...
export const CodeBlock = memo(function CodeBlock({
  ...
  return (
    ...
    >
      ...
      {/* Hover copy button (headless mode) */}
      {showCopyButton && (
        <button
          onClick={handleCopy}
          className="cursor-pointer absolute top-2 right-2 p-1.5 rounded-lg bg-background hover:bg-background border border-border opacity-0 group-hover/codeblock:opacity-100 transition-opacity z-10"
      ...
      <pre className="flex-1 min-h-0 overflow-auto p-4">
        <code className={cn('grid font-mono', textSize || 'text-base')}>
          {lines.map((line, i) => (
            <span
              key={i}
              className={cn(
...
export const CompactCodeBlock = memo(function CompactCodeBlock({
  ...
  return (
    <div
      className={cn(
        'relative group/codeblock block bg-muted rounded-lg max-w-full min-w-0 p-3',
        className
      )}
    >
      <button
          ...
          {lines.map((line, i) => (
            <span key={i} className="grid grid-cols-[auto_1fr] gap-4">
              <span
                className={cn(
                  textSize,
                  'text-muted-foreground/50 select-none text-right min-w-[2ch] leading-6'
                )}
              >
                {i + 1}
              </span>
