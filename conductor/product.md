...
TAOR engine (shipped 2026-03-19): Complete agent engine rewrite. Legacy 1,100-line engine.ts replaced with clean Think-Act-Observe-Reflect loop (`taor-loop.ts`). Unbounded iterations (no 15-iteration cap). Pre-flight checks and tool execution extracted to standalone modules. Old engine becomes thin re-export shim. Sub-agent decomposition via `spawn_agent` tool for isolated parallel domain queries. Deferred tool loading — eager core tools at start, growth tools loaded on demand. Sub-agent events visible in UI.

SOTA chat UX (shipped 2026-03-25): 10 new chat features — syntax-highlighted code blocks with shiki + copy button, regenerate response, thumbs up/down feedback, follow-up suggestion chips, artifact/canvas slide-out panel for code and HTML preview, voice input via Web Speech API, slash command palette, message editing with conversation forking, conversation search with history API, export menu (copy markdown, download md/json).

LeadSwarm outreach (in progress): Native lead discovery and email outreach integration. Supabase tables for email_templates, email_campaigns, campaign_leads. API routes for campaign CRUD, template management, prospect discovery. Email sending via Resend with plan-limit enforcement. Outreach dashboard with campaign metrics. Prospect discovery panel. Outreach intelligence panel with opportunity analysis by category (SEO, Tracking, Conversion, Technical). Website signals panel, score breakdown, next action recommendations.

Monochrome login page with force field background animation + branded Supabase auth emails. Collective 'we/us' voice across all dashboard UI. Critical multi-tenancy security fix: org_id isolation enforced on 8 API routes. Memory system unified on memory_palace_entries table.
...
**Channel smoke tests (T011).** All 5 key platform credentials are configured. Remaining: smoke test each channel adapter against real credentials, load test under concurrent access.

**Onboarding flow (T010).** Core routing and wizard implemented. Needs end-to-end verification.
...
**WhatsApp production (T008/T012).** WABA phone registration requires real mobile SIM (Meta blocks VoIP). Cheap prepaid AU SIM sufficient.

**Total Recall remaining (T031).** Phase 5: migrate WhatsApp/SMS/email channel adapters through the unified conversation pipeline. Phase 7: deprecate legacy ConversationRouter, identity backfill.

**Context Baseplate remaining (T009).** Knowledge graph persistence (currently in-memory only). No-reply contact cleanup.
...
personal-assistant/          Next.js 16 app (Vercel deployed dashboard)
  src/app/api/               25+ API route groups, 23 cron routes
  src/components/            17 component groups (incl. medications, whispers, leads)
  src/lib/agent/             TAOR engine, tools (26 across 6 groups), planner, prompt-builder, sub-agents, approval-queue, action-executor
  ...
  src/lib/leads/             Lead scoring, enrichment, discovery, outreach, campaigns, plan-limits
  supabase/migrations/       120 migrations
